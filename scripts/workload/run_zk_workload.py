...
ENDPOINTS = ["127.0.0.1:22181", "127.0.0.1:22182", "127.0.0.1:22183"]


def execute_op(clients: list[KazooClient], is_write: bool, key: str, value: str) -> bool:
    all_ok = True
    for c in clients:
        try:
            path = f"/{key}"
            if is_write:
                if c.exists(path):
                    c.set(path, value.encode("utf-8"))
                else:
                    c.create(path, value.encode("utf-8"), makepath=True)
            else:
                c.get(path)
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run ZooKeeper workload (80%% write / 20%% read)")
    ...
    clients: list[KazooClient] = []
    for ep in selected:
        cli = KazooClient(hosts=ep, timeout=2.0)
        try:
            cli.start(timeout=3)
            clients.append(cli)
